...
void EnzoMethodMultipole::compute ( Block * block) throw()
{
  ...
  *mass = 0;
  ...
  for (int i = 0; i < 3; i++){
    com[i]  = 0;   // replace with pcom[i] shorthand?
  }
...
{
  //CkPrintf("made it to prolong_send\n");
  ...
  while (it_child.next(ic3)) {
    ...
    //CkPrintf("in child loop\n");

    MultipoleMsg * msg = pack_coeffs_(enzo_block);
    //CkPrintf("packed coeffs\n");
    //CkPrintf("message c1[0]: %f\n", msg->c1[0]);
...
(EnzoBlock * enzo_block, MultipoleMsg * msg) throw()
{
  ...
  //std::string string = enzo_block->name();
  //CkPrintf("receiving block name = %s\n", string.c_str());
  //fflush(stdout);
  
  // CkPrintf("made it to prolong_recv\n");
  //CkPrintf("message com[0] in prolong_recv: %f\n", msg->com[0]);

  // Save multipole message from parent
  if (msg != nullptr)
    {
      //CkPrintf("is msg a nullptr?\n");
  ...
  *pmsg_prolong(enzo_block) = nullptr;
  ...
  //CkPrintf("right before unpacking coeffs\n");
...
(EnzoBlock * enzo_block, MultipoleMsg * msg) throw()
{
  ...
  // CkPrintf("this c1: %f %f %f\n", this_c1[0], this_c1[1], this_c1[2]);
  // CkPrintf("unpacked c1: %f %f %f\n", parent_c1[0], parent_c1[1], parent_c1[2]);

  // CkPrintf("this c2: ");
  //   for (int i = 0; i < 9; i++) {
  //     CkPrintf("%f  ", this_c2[i]);
  //   }
  // CkPrintf("\n");
...
(EnzoBlock * enzo_block, Index index_b, int type_b)
{
  ...
  if (index_a == index_b) {
    ...
    // CkPrintf("equal indices\n");
    // CkPrintf("level_a, level_b: %d %d\n", level_a, level_b);
    ...
    // CkPrintf("a is bigger than b\n");

    while (it_child_a.next(ica3)) {
      Index index_a_child = index_a.index_child(ica3, 0);
      enzo::block_array()[index_a_child].p_method_multipole_traverse
        (index_b,type_b);   // not confident about this
    }
  }
}
...
// replace with code to compute direct forces between leaf nodes?
 ...
 Index index_b, int volume_b)
{
  ...
  // compute the a->b interaction
  MultipoleMsg * msg_a = pack_dens_(enzo_block);
  enzo::block_array()[index_b].p_method_multipole_interact_direct (msg_a);
...
void EnzoBlock::p_method_multipole_interact_direct (MultipoleMsg * msg)
{
  EnzoMethodMultipole * method = static_cast<EnzoMethodMultipole*> (this->method());
...
{
  MultipoleMsg * msg = pack_dens_(enzo_block);
  enzo::block_array()[receiver].p_method_multipole_interact_direct(msg);
}


MultipoleMsg * EnzoMethodMultipole::pack_dens_(EnzoBlock * enzo_block) throw()
{
 
  // Create a MultipoleMsg for sending cell data

  std::string string = enzo_block->name();
  CkPrintf("block name = %s\n", string.c_str());
  ...
  enzo_block->lower(msg->lo, msg->lo+1, msg->lo+2);

  CkPrintf("before resize\n");
  CkPrintf("num cells: %d %d %d\n", mx, my, mz);

  msg->dens.resize((mx-2*gx) * (my-2*gy) * (mz-2*gz));

  CkPrintf("dens size: %d\n", msg->dens.size());
...
void EnzoMethodMultipole::interact_direct_(Block * block, MultipoleMsg * msg_b) throw()
{
        ...
        for (int iz2 = gz; iz2 < mz-gz; iz2++) {
          for (int iy2 = gy; iy2 < my-gy; iy2++) {
            for (int ix2 = gx; ix2 < mx-gx; ix2++) {
              ...
              std::vector<double> disp (3, 0);
              disp[0] = msg_b->lo[0] - lo[0] + (ix2 - ix)*hx; 
              disp[1] = msg_b->lo[1] - lo[1] + (iy2 - iy)*hy;
              disp[2] = msg_b->lo[2] - lo[2] + (iz2 - iz)*hz;
                
              std::vector<double> cell_force = newton_force_(msg_b->dens[i2]*cell_vol, disp); 
...
(EnzoBlock * enzo_block, Index index, int volume)
{
  ...
  if (enzo_block->is_leaf()) {
    ...
    *volume_(enzo_block) += volume;
    ...
    while (it_child.next(ic3)) {
      // CkPrintf("update volume child loop");
